...
// --- Helper functions for matrix-based action indexing ---
...
Node* First_InsertFrontier_Search_TREE(const enum METHODS method, Node *const root, State *const goal_state, float alpha)
{
	...
	// Print_Frontier(frontier);	                      DEBUG
    
    explorer_set =  New_Hash_Table(HASH_TABLE_BASED_SIZE);
	// Show_Hash_Table(explorer_set);                     DEBUG
			...
			if(child!=NULL){
				...
				switch(method){
					...
					case GeneralizedAStarSearch:
    					if(temp_node!=NULL){
        					float old_eval = alpha * temp_node->path_cost + (1 - alpha) * temp_node->state.h_n;
        					float new_eval = alpha * child->path_cost + (1 - alpha) * child->state.h_n;
...
Node* DepthType_Search_TREE(const enum METHODS method, Node *const root, State *const goal_state, const int Max_Level)
{
    ...
    double max_duration = 3600.0; // 1 hour = 3600 seconds
    ...
    while (Number_Searched_Nodes < MAX_SEARCHED_NODE)
    {
        ...
        if (elapsed_time > max_duration) {
            printf("\nTIMEOUT: DFS ran for 1 hour (%0.2f seconds) but found no solution.\n", elapsed_time);
        	printf("\nThe number of searched nodes is : %d\n", Number_Searched_Nodes);
			printf("\nThe number of generated nodes is : %d\n", Number_Generated_Nodes);
			printf("\nThe number of generated nodes in memory is : %d\n", Number_Allocated_Nodes);
            Delete_Hash_Table(explorer_set);
    ...
    printf("\n Limit exceeded: %d node searched but no solution found.\n", Number_Searched_Nodes);
    Delete_Hash_Table(explorer_set);
    return FAILURE;
}
...
Node* Child_Node(Node *const parent, const enum ACTIONS action)
{
      ...
      if( Result(&(parent->state), action, &trans_model) ){
          ...
          child->parent    = parent;
          child->action    = Get_Action_Enum_Index(action);
...
Node* Pop(Queue **frontier)
{
	...
	/* printf("\nPOP: ");
    Print_Node(node);			for NODE DEBUG
    printf("\n"); */
...
void Insert_Priority_Queue_GENERALIZED_A_Star(Node *const child, Queue **frontier, float alpha) {
    ...
    new_queue->node = child;
    float child_eval = alpha * child->path_cost + (1 - alpha) * child->state.h_n;

    if (Empty(*frontier)) {
        new_queue->next = NULL;
        *frontier = new_queue;
    } else {
        float frontier_eval = alpha * (*frontier)->node->path_cost + (1 - alpha) * (*frontier)->node->state.h_n;
        if (child_eval < frontier_eval) {
            new_queue->next = *frontier;
            *frontier = new_queue;
        } else {
            for (temp_queue = *frontier; temp_queue->next != NULL; temp_queue = temp_queue->next) {
                float next_eval = alpha * temp_queue->next->node->path_cost + (1 - alpha) * temp_queue->next->node->state.h_n;
                if (child_eval < next_eval) {
                    new_queue->next = temp_queue->next;
                    temp_queue->next = new_queue;
                    return;
                }
